...
if pcregrep --color -nr --include='\.adoc' -e 'Fluid|Fluidsignal\ Group|fluidsignal|\ fluid[)}\ \]]' content; then echo -e "${GC}\nThe only accepted name is FLUID.${NC}"; ERRORS=1; fi

# Check blank spaces after headers
if pcregrep --color -Mrn --include='\.adoc' '^=.*.[A-Z].*.*\n.*[A-Z]' content ; then echo -e "${GC}\nLeave a blank space after a header.${NC}"; ERRORS=1; fi

# Check that the references are numerated
if pcregrep --color -Mnr '^== Referenc.*.*\n.*\n[A-Za-z]' content; then echo -e "${GC}\nReferences must be numbered.${NC}"; ERRORS=1;fi

# Check there are not any articles with the .asc extension
if find content -iname '*.asc' | egrep '.*'; then echo -e "${GC}Extension \".asc\" not supported.${NC}"; ERRORS=1;fi

# Check that names do not have underscore
if find content -iname '*_*' | egrep '.*'; then echo -e "${GC}Use hyphen '-' instead of underscore '_' for filenames.${NC}"; ERRORS=1;fi

# Check every image is in PNG format
if find . -name '*.jpg' -o -name '*.jpeg' -o -name '*.svg' | egrep '.*'; then echo -e "${GC}Image format must be \"png\".${NC}"; ERRORS=1;fi

# Check every image fits the size limit
if find . -name '*.png' -size +300k | egrep '.*'; then echo -e "${GC}Images cannot have a size over 300kB.${NC}"; ERRORS=1;fi

# Check that the image caption is not manually placed
if pcregrep --color -nr --include='\.adoc' -e '^\.Imagen?\ [0-9]|^\.Figur(a|e)\ [0-9]' content; then echo -e "${GC}Images must not have the words \"Image #\" or \"Figure #\" in their captions.${NC}"; ERRORS=1;fi

# Check the titles of the images are well placed
if pcregrep --color -Mnr --include='\.adoc' 'image::.*\n\.[a-zA-Z]' content; then echo -e "${GC}Image captions are placed after the image, not before.${NC}"; ERRORS=1;fi

# Check no uppercase characters are used in the filenames
if find content | egrep '.*[A-Z].*'; then echo -e "${GC}Filenames must always be lowercase.${NC}"; ERRORS=1;fi

# Check that filenames do not have spaces in them
if find content -iname '* *' | egrep '.*'; then echo -e "${GC}Filenames must not have spaces in them, use hyphen \"-\" instead.${NC}"; ERRORS=1;fi

# Check that slugs have under 44 characters
if grep -E -n -r --include "*.adoc" "^:slug: .{44,}" content; then echo -e "${GC}The \"slug\" can have 43 characters maximum.${NC}"; ERRORS=1;fi

# Check that 4 '-' delimit the code block, not more, not less
if  pcregrep --color -nr --include='\.adoc' '^-{5,}' content; then echo -e "${GC}Code blocks must be delimited by exactly four hyphens '-'.${NC}"; ERRORS=1;fi

# Check that the start attribute is never used
if pcregrep --color -nr --include='\.adoc' '\[start' content; then echo -e "${GC}Do not use the \"start\" attribute for list numbering, use a plus sing '+' to concatenate the content that breaks the numbering.${NC}"; ERRORS=1;fi

# Check that the slug ends in a '/'
if pcregrep --color -nr --include='\.adoc' '^:slug:.*[a-z]$' content; then echo -e "${GC}The \"slug\" must end in '/'.${NC}"; ERRORS=1;fi

# Check alternative text in images
if pcregrep --color -Mnr --include='\.adoc' 'image\:\:.*\[\]' content; then echo -e "${GC}Images do not have alt attribute.${NC}"; ERRORS=1;fi

# Check double main title
if pcregrep -M -n -r --include='\.adoc$' '^= .*\n\n.*^= .*\n\n' content; then -e echo "${GC}Double main title.${NC}"; ERRORS=1;fi

# Check double quotes are not used in the title
if pcregrep --color -nr --include='\.adoc' '^= [A-Z].*\"' content; then echo -e "${GC}Do not use double quotes \" in the title.${NC}"; ERRORS=1;fi

# Check that blog articles have alt description for their featured images
if pcregrep --color -Lnr --include='\.adoc' '^:alt:.*' content/blog*; then echo -e "${GC}The articles must have the metadata \"alt\" set for their representative image.${NC}"; ERRORS=1;fi

# Check that code does not follow inmmediatly after a paragraph in the KB
if pcregrep --color -Mnr --include='\.adoc' '^[a-zA-Z0-9].*\n.*\[source' content/defends; then echo -e "${GC}Source codes must be separated from a paragraph using a plus sign '+'.${NC}"; ERRORS=1;fi

# Check that the title of the website does not have more than 60 characters (Once "| FLUID" is attached)
if pcregrep --color -nru --include='\.adoc' '^= [A-Z¿¡].{52}' content; then echo -e "${GC}Titles can have 52 characters maximum.${NC}"; ERRORS=1;fi

# Check that every .adoc has keywords defined
if pcregrep -Lnr --include='\.adoc' ':keywords:' content; then echo -e "${GC}The attribute \"keywords\" must be defined in every .adoc.${NC}"; ERRORS=1;fi

# Check that the every .adoc has description defined
if pcregrep -Lnr --include='\.adoc' ':description:' content; then echo -e "${GC}The attribute \"description\" must be defined in every .adoc.${NC}"; ERRORS=1;fi
