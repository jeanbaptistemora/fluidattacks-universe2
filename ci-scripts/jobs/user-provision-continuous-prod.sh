...
user_provision_continuous_prod_rotate_aws() {
  ...
  set_project_variable "$GITLAB_API_TOKEN" "$CONTINUOUS_REPO_ID" PROD_AWS_ACCESS_KEY_ID "$VAR_KEY" true false
  set_project_variable "$GITLAB_API_TOKEN" "$CONTINUOUS_REPO_ID" PROD_AWS_SECRET_ACCESS_KEY "$VAR_SECRET" true false
}
