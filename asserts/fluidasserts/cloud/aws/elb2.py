    ...
    if balancers:
        for balancer in balancers:
            ...
            for listener in aws.run_boto3_func(
                ...
                (vulns if listener["Protocol"] == "HTTP" else safes).append(
                    (load_balancer_arn, "protocol must be HTTPS")
                )
...
@api(risk=HIGH, kind=DAST)
@unknown_if(BotoCoreError, RequestException)
def uses_insecure_ssl_protocol(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    """
            ...
            for listener in aws.run_boto3_func(
                ...
                if listener.get("SslPolicy", ""):
                    ...
                    for protocol in policy[0]["SslProtocols"]:
                        (
                            vulns
                            if protocol not in acceptable_protos
                            else safes
                        ).append(
                            (
...
def uses_insecure_security_policy(
    ...
    msg_closed: str = (
        "ELB Load Balancers does not implement " "insecure security policy"
    )
        ...
        for balancer in balancers:
            ...
            for listener in aws.run_boto3_func(
                ...
                if listener.get("SslPolicy", ""):
                    (
                        vulns
                        if listener.get("SslPolicy", "") not in acceptable
                        else safes
                    ).append(
                        (
                            f'{listener["LoadBalancerArn"]}/'
