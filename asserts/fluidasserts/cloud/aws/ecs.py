...
def has_not_resources_usage_limits(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    """
    ...
    for task in tasks:
        ...
        (vulns if vulnerable else safes).append(
            (
                task,
                (
                    "Set a resource usage limit in the defined"
...
def write_root_file_system(
    ...
    for task in tasks:
        ...
        vulnerable = any(
            map(
                lambda x: not x["readonlyRootFilesystem"]
                if "readonlyRootFilesystem" in x.keys()
                else True,
        ...
        (vulns if vulnerable else safes).append(
            (
                task,
                (
                    "Set readonlyRootFilesystem property as true in"
...
def no_iam_role_for_tasks(
    ...
    for task in tasks:
        ...
        (
            vulns if "taskRoleArn" not in task_description.keys() else safes
        ).append((task, "Set an IAM role for the task."))
...
def run_containers_as_root_user(
    ...
    for task in tasks:
        ...
        vulnerable = any(
            map(
                lambda x: True
                if "user" not in x.keys()
                else _is_root(x["user"]),
                task_description["containerDefinitions"],
            )
        )
        (vulns if vulnerable else safes).append(
    ...
    for task in tasks:
        ...
        vulnerable = any(
            _flatten(
                list(
                    map(
                        lambda x: list(
                            map(lambda y: not y["readOnly"], x["mountPoints"])
                        ),
                        task_description["containerDefinitions"],
                    )
                )
        ...
        (vulns if vulnerable else safes).append(
            (
                task,
                (
                    "Set property mountPoints.readOnly as true in the"
