    ...
    for cluster in clusters:
        ...
        if vol_key:
            ...
            (vulns if vulnerable else safes).append(
                (cluster["ClusterIdentifier"], "uses default KMS key")
            )
    return _get_result_as_tuple(
        service="RedShift",
        objects="clusters",
...
def is_audit_logging_disabled(
    ...
    for cluster in clusters:
        ...
        (vulns if not logging else safes).append(
            (cluster_id, "uses default KMS key")
        )
    return _get_result_as_tuple(
        service="RedShift",
        objects="clusters",
...
def is_not_upgrade_allowed(
    ...
    for cluster in clusters:
        ...
        (
            vulns if not cluster.get("AllowVersionUpgrade", False) else safes
        ).append((cluster_id, "have Allow Version Upgrade disabled"))
    return _get_result_as_tuple(
        service="RedShift",
        objects="clusters",
        msg_open=msg_open,
...
@unknown_if(BotoCoreError, RequestException)
def is_user_activity_logging_disabled(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
