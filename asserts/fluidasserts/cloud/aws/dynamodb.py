...
def encrypted_with_aws_master_keys(
    ...
    """
    msg_open: str = (
        "DynamoDB tablas uses AWS-owned Master Keys for"
        "Server-Side Encryption."
    )
    msg_closed: str = (
        "DynamoDB tablas uses Customer-owned Master Keys"
...
@api(risk=MEDIUM, kind=DAST)
@unknown_if(BotoCoreError, RequestException)
def has_disabled_continuous_backups(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    """
    ...
    for table in table_names:
        ...
        vulnerable = (
            table_backup["PointInTimeRecoveryDescription"][
