        ...
        for topic in topics:
            ...
            for statement in policy["Statement"]:
                if (
                    ...
                    (
                        vulns if not statement.get("Condition", {}) else safes
                    ).append((topic_arn, "is publishable by anyone"))
        ...
        for topic in topics:
            ...
            for statement in policy["Statement"]:
                if (
                    ...
                    (
                        vulns if not statement.get("Condition", {}) else safes
                    ).append((topic_arn, "is subscribable by anyone"))
...
@unknown_if(BotoCoreError, RequestException)
def is_server_side_encryption_disabled(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    ...
    if topics:
        for topic in topics:
            ...
            (vulns if not attrs.get("KmsMasterKeyId", "") else safes).append(
                (topic_arn, "has serverside encryption disabled")
            )
...
def uses_default_kms_key(
    ...
    if topics:
        for topic in topics:
            ...
            ).append((topic_arn, "uses default KMS key"))
