...
def uses_default_port(
    ...
    for cluster in caches:
        ...
        ).append((cluster["CacheClusterId"], "uses default port"))
...
@unknown_if(BotoCoreError, RequestException)
def uses_unsafe_engine_version(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
...
@unknown_if(BotoCoreError, RequestException)
def is_transit_encryption_disabled(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    ...
    msg_closed: str = (
        "Elasticache clusters do not have transit " "encryption disabled"
    )
    ...
    for cluster in caches:
        (
            ...
            (cluster["CacheClusterId"], "has transit encryption disabled")
        )
...
@api(risk=HIGH, kind=DAST)
@unknown_if(BotoCoreError, RequestException)
def is_at_rest_encryption_disabled(
    key_id: str, secret: str, session_token: str = None, retry: bool = True
) -> tuple:
    ...
    msg_closed: str = (
        "Elasticache clusters do not have at rest " "encryption disabled"
    )
    ...
    for cluster in caches:
        (
            ...
            (cluster["CacheClusterId"], "has At Rest encryption disabled")
        )
