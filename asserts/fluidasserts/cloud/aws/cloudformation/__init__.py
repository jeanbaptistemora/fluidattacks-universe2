...
Vulnerability = namedtuple(
    "Vulnerability",
    ["path", "entity", "identifier", "reason", "line"],
...
def _get_result_as_tuple(
    *, vulnerabilities: List[Vulnerability], msg_open: str, msg_closed: str
) -> tuple:
    ...
    vuln_units: List[Unit] = [
        Unit(
            where=f"{x.path} - [{x.identifier}] {x.entity} {x.reason}.",
            specific=[x.line],
        )
...
def get_ref_nodes(
    graph: DiGraph, node: int, condition: Callable = None, depth: int = 3
) -> List[int]:
    """
    ...
    if not condition:
        ...
        def condition(value):  # noqa pylint: disable=function-redefined
            return True  # noqa pylint: disable=unused-argument,multiple-statements

    nodes: List[int] = (
        [node]
        if "value" in graph.nodes[node]
        and condition(graph.nodes[node]["value"])
        else [
            ref
            for ref in dfs_preorder_nodes(graph, node, depth)
...
def get_ref_inverse(
    graph: DiGraph, node: int, labels: Union[Set[str], str], depth: int = 3
):
    """Returns nodes that reference target node."""
    root: int = get_predecessor(graph, node, "Resources")
    refs = has_values(
        graph, root, "Ref", [graph.nodes.get(node)["name"]], depth=depth + 1
    )
...
def get_predecessor(
    graph: DiGraph, node: int, labels: Union[Set[str], str]
) -> int:
    """
...
def get_templates(
    ...
    templates: List[Tuple[int, Dict]] = [
        (_id, node)
        for _id, node in graph.nodes.data()
...
def get_resources(
    ...
    info: bool = False,
    num_labels: int = None,
) -> Union[Tuple[int, Dict, Dict], List[int]]:
    """
    ...
    if info:
        return [
            (
                ...
                graph.nodes[node],
                graph.nodes[
                    get_predecessor(graph, node, "CloudFormationTemplate")
                ],
            )
            for x in nodes
            for node in dfs_preorder_nodes(graph, x, depth)
            if len(graph.nodes[node]["labels"].intersection(labels))
            >= num_labels
        ]
    return [
        node
...
def get_list_node_items(graph: DiGraph, node: int, label: str, depth: int):
    ...
    _items = helper.get_index(
        get_resources(graph, node, label, depth=depth), 0
    )
    ...
    if value:
        ...
        items = get_resources(graph, _items, "Item", depth=depth)
    return items


def policy_statement_privilege(
    graph: DiGraph, statement: int, effect: str, action: str
):
...
def policy_actions_has_privilege(
    graph: DiGraph, action: int, privilege
) -> bool:
    ...
    if "value" in action_node:
        if action_node["value"] == "*":
            ...
            if act.startswith("*"):
                ...
                actions.append(
                    act in write_actions.get(serv, {}).get(privilege, [])
                )
            success = any(actions)
...
def get_value(graph: DiGraph, node: int):
