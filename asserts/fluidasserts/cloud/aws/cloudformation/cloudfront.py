...
def serves_content_over_http(
    ...
    for dist, resource, template in distributions:
        ...
        found_issues: list = []
        if view_policy:
            view_policy_node = get_ref_nodes(
                graph, view_policy, lambda x: isinstance(x, str)
            )[0]
            value = graph.nodes[view_policy_node]
            if any(map(value["value"].__eq__, vulnerable_protocol_policy)):
                found_issues.append(
                    ("DefaultCacheBehavior", value["value"], value["line"])
                )

        behaviors = get_resources(graph, dist, "CacheBehaviors", 3)
        view_policies = get_resources(
            graph, behaviors, "ViewerProtocolPolicy", 4
        )
        for policy in view_policies:
            policy_node = get_ref_nodes(
                graph, policy, lambda x: isinstance(x, str)
            )[0]
            value = graph.nodes[policy_node]
            if any(map(value["value"].__eq__, vulnerable_protocol_policy)):
                found_issues.append(
                    ("CacheBehaviors", value["value"], value["line"])
                )
    ...
    for dist, resource, template in distributions:
        ...
        if version:
            version_node = helper.get_index(
                get_ref_nodes(
                    graph, version, lambda x: x in vulnerable_min_prot_versions
                ),
                0,
            )
            if version_node:
