...
def has_terminate_shutdown_behavior(
    ...
    for l_templates in launch_templates:
        ...
        template = graph.nodes[
            get_predecessor(graph, l_templates, "CloudFormationTemplate")
        ]
        behavior = [
            node
            for node in dfs_preorder_nodes(graph, l_templates, 12)
            if "InstanceInitiatedShutdownBehavior"
            in graph.nodes[node]["labels"]
        ]
        if not behavior:
            continue
        behavior_node = behavior[0]
        behavior_node = get_ref_nodes(
            graph, behavior_node, lambda x: x == "terminate"
        )
        if behavior_node:
            _type = get_type(
                graph, l_templates, {"Instance", "LaunchTemplate"}
            )
            vulnerabilities.append(
                Vulnerability(
                    path=template["path"],
    ...
    return _get_result_as_tuple(
        vulnerabilities=vulnerabilities,
        msg_open=(
            "EC2 Launch Templates allows the shutdown command to"
            " terminate the instance"
        ),
        msg_closed=(
            "EC2 Launch Templates disallow the shutdown command to"
            " terminate the instance"
        ),
    )
...
def is_associate_public_ip_address_enabled(
    ...
    public_ips = [
        node
        for net in networks
        for node in dfs_preorder_nodes(graph, net, 3)
        if "AssociatePublicIpAddress" in graph.nodes[node]["labels"]
    ]

    for _ip in public_ips:
        resource = graph.nodes[get_predecessor(graph, _ip, "Instance")]
        template = graph.nodes[
            get_predecessor(graph, _ip, "CloudFormationTemplate")
        ]
        _ip_node = get_ref_nodes(
            graph, _ip, lambda x: x in (True, "true", "True", "1", 1)
        )
        if _ip_node:
...
def uses_default_security_group(
    ...
    **SecurityGroups** or **SecurityGroupIds** are launched with the default
    ...
    for l_template in launch_templates:
        ...
        seg_group = [
            node
            for node in dfs_preorder_nodes(graph, l_template, 6)
            if graph.nodes[node]["labels"].intersection(
        ...
        if not seg_group:
            vulnerabilities.append(
                Vulnerability(
                    path=template["path"],
                    entity=("AWS::EC2::Instance/" "SecurityGroups(Ids)"),
                    identifier=resource["name"],
                    line=resource["line"],
                    reason=(
