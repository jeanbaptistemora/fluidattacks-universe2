...
def has_key_expiration_disabled(
    ...
    key_vaults = KeyVaultManagementClient(
...
def entities_have_all_access(
    ...
    msg_closed: str = (
        "Users, groups and applications can not perform all"
        " management actions."
    )
    vulns, safes = [], []

    rules = {
        "certificates": lambda x: len(x) == 16,
...
@api(risk=MEDIUM, kind=DAST)
@unknown_if(ClientException, AuthenticationError, HttpResponseError)
def has_secret_expiration_disabled(
    client_id: str, secret: str, tenant: str, subscription_id: str
) -> Tuple:
    """
