...
def _get_result_as_tuple(
    *,
    ...
    if vulns:
        vuln_units.extend(
            Unit(where=f"Azure{id_}", specific=[vuln]) for id_, vuln in vulns
        )
    if safes:
        safe_units.extend(
            Unit(where=f"Azure/{id_}", specific=[safe]) for id_, safe in safes
        )
...
def _get_credentials(
    client_id: str, secret: str, tenant: str
) -> ServicePrincipalCredentials:
