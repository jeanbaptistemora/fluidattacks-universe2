...
def has_user_managed_account_keys(
    project_id: str, cred_file: str, retry: bool = True
) -> tuple:
    """
    ...
    """
    serv_accounts = gcp.get_service_accounts(
        project_id=project_id, credentials_file=cred_file, retry=retry
    )
    ...
    for user in serv_accounts:
        user_managed_keys = gcp.get_keys_managed_by_user(
            user, credentials_file=cred_file, retry=retry
        )
    ...
    if mngd_keys:
        vulns.append(
            (",".join(mngd_keys), "Must not be managing service keys")
        )
