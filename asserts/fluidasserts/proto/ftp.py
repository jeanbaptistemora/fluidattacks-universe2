...
def _ftp_do_auth(
    *,
    ...
    unit: Unit = Unit(
        where=f"FTP://{username}:{password}@{ip_address}:{port}",
...
def is_a_valid_user(
    ip_address: str, username: str, password: str, port: int = PORT
) -> tuple:
    """
    ...
    return _ftp_do_auth(
        ...
        username=username,
        password=password,
        msg_open="Credentials are valid",
        msg_closed="Credentials are invalid",
    )


@api(risk=HIGH, kind=DAST)
def user_without_password(
    ip_address: str, username: str, port: int = PORT
) -> tuple:
    """
    ...
    return _ftp_do_auth(
        ...
        msg_open="Password is not required",
        msg_closed="Password is required",
    )
