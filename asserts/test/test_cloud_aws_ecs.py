...
def test_has_not_resources_usage_limits_open():
    ...
    assert ecs.has_not_resources_usage_limits(
...
def test_write_root_file_system_open():
    ...
    assert ecs.write_root_file_system(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_no_iam_role_for_tasks_open():
    ...
    assert ecs.no_iam_role_for_tasks(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_run_containers_as_root_user_open():
    ...
    assert ecs.run_containers_as_root_user(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
