...
def upload_file_secure():
    ...
    if my_file and allowed_file(my_file.filename):
        my_file.save(
            os.path.join(APP.config["UPLOAD_FOLDER"], my_file.filename)
        )
...
def brute_force_get_fail():
    ...
    if (
        request.values["username"] == "admin"
        and request.values["password"] == "password"
    ):
...
def brute_force_json_fail():
    """Form con brute forcing."""
    if (
        request.get_json()["username"] == "admin"
        and request.get_json()["password"] == "password"
    ):
        return "You are admin now"
    return "Login incorrect"
...
def session_fixation_open():
    ...
    return redirect(
        url_for("session_fixated_vuln", sessionid=12345678), code=302
    )
...
def session_fixation_close():
    ...
    return redirect(
        url_for("session_fixated_not_vuln", sessionid=12345678), code=302
    )
...
def http_only_fail():
    ...
    resp = Response("Login successful")
    resp.set_cookie(
        "JSESSID", "World", secure=True, httponly=False, samesite="lax"
    )
    return resp
...
def http_only_ok():
    ...
    resp = Response("Login successful")
    resp.set_cookie(
        "JSESSID", "World", secure=True, httponly=True, samesite="strict"
    )
    return resp
