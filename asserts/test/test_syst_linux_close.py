...
def test_min_priv_enabled_close(get_mock_ip):
    """Secure umask?."""
    assert not linux.is_min_priv_disabled(
        get_mock_ip, NONPRIV_USER, NONPRIV_PASS
    )
    assert not linux.is_min_priv_disabled(
...
def test_compilers_installed_close(get_mock_ip):
    """Compiler installed?."""
    assert not linux.are_compilers_installed(
        get_mock_ip, NONPRIV_USER, NONPRIV_PASS
    )
    assert not linux.are_compilers_installed(
        NON_EXISTANT, NONPRIV_USER, NONPRIV_PASS
...
def test_antimalware_installed_close(get_mock_ip):
    ...
    assert not linux.is_antimalware_not_installed(
        get_mock_ip, NONPRIV_USER, NONPRIV_PASS
    )
    assert not linux.is_antimalware_not_installed(
        NON_EXISTANT, NONPRIV_USER, NONPRIV_PASS
...
def test_remote_admin_enabled_close(get_mock_ip):
    ...
    assert not linux.is_remote_admin_enabled(
        get_mock_ip, NONPRIV_USER, NONPRIV_PASS
    )
    assert not linux.is_remote_admin_enabled(
        NON_EXISTANT, NONPRIV_USER, NONPRIV_PASS
...
def test_syncookies_enabled_close(get_mock_ip):
    """SYN Cookies enabled?."""
    assert not linux.are_syncookies_disabled(
        get_mock_ip, NONPRIV_USER, NONPRIV_PASS
    )
    assert not linux.are_syncookies_disabled(
        NON_EXISTANT, NONPRIV_USER, NONPRIV_PASS
