...
def test_ok():
    ...
    with NamedTemporaryFile(mode="w") as encrypted_file, NamedTemporaryFile(
        mode="r"
    ) as decrypted_file:
        ...
        assert crypto.create_encrypted_yaml(
            key_b64=key_b64_good__, secrets=secrets_good, file=encrypted_file
        )
        ...
        secrets = crypto.DecryptedYAML(
            key_b64=key_b64_good__, encrypted_yaml_path=encrypted_file.name
        )
        ...
        assert crypto.create_decrypted_yaml(
            key_b64=key_b64_good__,
        ...
        assert crypto.create_decrypted_yaml(
            key_b64=key_b64_good__, input_file=encrypted_file.name
        )
...
def test_read():
    ...
    # Load the encrypted yaml file
    secrets = crypto.DecryptedYAML(
        key_b64=key_b64_good__, encrypted_yaml_path=secrets_file_path
    )
...
def test_bad():
    ...
    with pytest.raises(AssertionError):
        assert crypto.create_encrypted_yaml(
            key_b64=key_b64_bad_29, secrets=secrets_good
        )

    with pytest.raises(AssertionError):
        assert crypto.create_encrypted_yaml(
            key_b64=key_b64_bad_33, secrets=secrets_good
        )

    with pytest.raises(AssertionError):
        assert crypto.create_encrypted_yaml(
            key_b64=key_b64_good__, secrets=secrets_bad
        )
