...
def test_local_infile_close(get_mock_ip):
    ...
    #                                         ADMIN_PASS)
    assert not mysql.local_infile_enabled(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)
...
def test_symlinks_enabled_close(get_mock_ip):
    ...
    #                                         ADMIN_PASS)
    assert not mysql.symlinks_enabled(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)


@pytest.mark.parametrize("get_mock_ip", ["mysql_db_hard"], indirect=True)
def test_memcached_enabled_close(get_mock_ip):
    """Check if MySQL memcached is enabled."""
    assert not mysql.memcached_enabled(get_mock_ip, ADMIN_USER, ADMIN_PASS)
    assert not mysql.memcached_enabled(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)
...
def test_secure_file_close(get_mock_ip):
    ...
    #                                      ADMIN_PASS)
    assert not mysql.secure_file_priv_disabled(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
...
def test_strict_all_tables_close(get_mock_ip):
    ...
    #                                      ADMIN_PASS)
    assert not mysql.strict_all_tables_disabled(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
...
def test_log_error_close(get_mock_ip):
    ...
    assert not mysql.log_error_disabled(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)
...
def test_logs_on_systemfs_close(get_mock_ip):
    ...
    #                                      ADMIN_PASS)
    assert not mysql.logs_on_system_fs(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)
...
def test_logs_verbosity_close(get_mock_ip):
    ...
    #                                      ADMIN_PASS)
    assert not mysql.logs_verbosity_low(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)


@pytest.mark.parametrize("get_mock_ip", ["mysql_db_hard"], indirect=True)
def test_auto_creates_users_close(get_mock_ip):
    """Check if MySQL auto creates users."""
    assert not mysql.auto_creates_users(get_mock_ip, ADMIN_USER, ADMIN_PASS)
    assert not mysql.auto_creates_users(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)


@pytest.mark.parametrize("get_mock_ip", ["mysql_db_hard"], indirect=True)
def test_users_without_pass_close(get_mock_ip):
    """Check if MySQL users have passwords."""
    assert not mysql.has_users_without_password(
        get_mock_ip, ADMIN_USER, ADMIN_PASS
    )
    assert not mysql.has_users_without_password(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
...
def test_password_expiration_close(get_mock_ip):
    ...
    assert not mysql.password_expiration_unsafe(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
...
def test_password_equals_to_user_close(get_mock_ip):
    ...
    assert not mysql.password_equals_to_user(
        get_mock_ip, ADMIN_USER, ADMIN_PASS
    )
    assert not mysql.password_equals_to_user(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
...
def test_wildcard_hosts_close(get_mock_ip):
    ...
    assert not mysql.users_have_wildcard_host(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
