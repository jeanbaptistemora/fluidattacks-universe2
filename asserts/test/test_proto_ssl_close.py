...
@pytest.mark.parametrize('get_mock_ip', ['ssl_hard'], indirect=True)
def test_raccoon_close(get_mock_ip):
    """Check Raccoon attack close."""
    assert not ssl.has_raccoon(get_mock_ip, SSL_PORT)


@pytest.mark.parametrize('get_mock_ip', ['ssl_hard'], indirect=True)
def test_raccoon_unknown(get_mock_ip):
    """Check Raccoon attack unknown."""
    assert not ssl.has_raccoon(get_mock_ip, SSL_PORT)
    assert not ssl.has_raccoon(get_mock_ip, 80)
    assert not ssl.has_raccoon(NON_EXISTANT, 80)
