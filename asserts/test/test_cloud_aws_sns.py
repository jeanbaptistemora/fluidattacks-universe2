...
def test_can_anyone_publish_open():
    ...
    assert sns.can_anyone_publish(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY  # pylint: disable=E1101
    ).is_open()


def test_can_anyone_subscribe_open():
    """Test sns.can_anyone_subscribe."""
    assert sns.can_anyone_subscribe(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY  # pylint: disable=E1101
    ).is_open()
...
def test_is_server_side_encryption_disabled_open():
    ...
    assert sns.is_server_side_encryption_disabled(
        AWS_ACCESS_KEY_ID,  # pylint: disable=E1101,C0301 # noqa: E501
...
def test_uses_default_kms_key_open():
    ...
    assert sns.uses_default_kms_key(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY  # pylint: disable=E1101
    ).is_open()
