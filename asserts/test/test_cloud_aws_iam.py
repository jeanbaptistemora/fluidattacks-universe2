...
def test_pass_len_unsafe_open():
    ...
    assert iam.min_password_len_unsafe(
...
def test_pass_reuse_unsafe_open():
    ...
    assert iam.password_reuse_unsafe(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)


def test_pass_expiration_unsafe_open():
    """Search IAM policy: Password expiration requirement."""
    assert iam.password_expiration_unsafe(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    )
...
def test_policies_attached_open():
    ...
    assert iam.policies_attached_to_users(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_have_old_access_keys_open():
    ...
    assert iam.have_old_access_keys(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_has_old_ssh_public_keys_close():
    ...
    assert iam.has_old_ssh_public_keys(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_closed()
    assert iam.has_old_ssh_public_keys(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY_BAD, retry=False
...
def test_has_permissive_role_policies_closed():
    ...
    assert iam.has_permissive_role_policies(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_closed()
    assert iam.has_permissive_role_policies(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY_BAD, retry=False
...
def test_has_mfa_disabled_close():
    ...
    assert not iam.has_mfa_disabled(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)
