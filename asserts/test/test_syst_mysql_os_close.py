...
def test_high_privileged_close(get_mock_ip):
    ...
    assert not mysql_os.daemon_high_privileged(
...
def test_history_enabled_close(get_mock_ip):
    ...
    assert not mysql_os.history_enabled(get_mock_ip, ADMIN_USER, ADMIN_PASS)
    assert not mysql_os.history_enabled(NON_EXISTANT, ADMIN_USER, ADMIN_PASS)
...
def test_has_insecure_shell_close(get_mock_ip):
    ...
    assert not mysql_os.has_insecure_shell(
        NON_EXISTANT, ADMIN_USER, ADMIN_PASS
    )
