...
def test_uses_default_port_open():
    ...
    assert elasticache.uses_default_port(
...
def test_uses_unsafe_engine_version_open():
    ...
    assert elasticache.uses_unsafe_engine_version(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_is_transit_encryption_disabled_open():
    ...
    assert elasticache.is_transit_encryption_disabled(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
...
def test_is_at_rest_encryption_disabled_open():
    ...
    assert elasticache.is_at_rest_encryption_disabled(
        AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
    ).is_open()
