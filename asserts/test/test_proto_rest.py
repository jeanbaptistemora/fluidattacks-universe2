...
pytestmark = pytest.mark.asserts_module("proto_rest")
...
def test_hsts_open():
    ...
    assert rest.is_header_hsts_missing("%s/hsts/fail" % (BASE_URL))
...
def test_content_options_open():
    ...
    assert rest.is_header_x_content_type_options_missing(
        "%s/content_options/fail" % (BASE_URL)
    ).is_open()
...
def test_insecure_accept_close():
    ...
    assert not rest.accepts_insecure_accept_header(
        BASE_URL + "/insecure_accept/ok", timeout=10
    )
    assert not rest.accepts_insecure_accept_header(
...
def test_hsts_close():
    ...
    assert not rest.is_header_hsts_missing("%s/hsts/ok" % (BASE_URL))
    assert not rest.is_header_hsts_missing(
        "%s/hsts/ok" % (NONEXISTANT_SERVICE)
    )
    assert not rest.is_header_hsts_missing("%s/hsts/ok" % (BAD_FORMAT_SERVICE))


def test_content_options_close():
    """Check X-Content-Type-Options header."""
    assert rest.is_header_x_content_type_options_missing(
        "%s/content_type/ok" % (BASE_URL)
    ).is_closed()
    assert rest.is_header_x_content_type_options_missing(
        "%s/content_type/ok" % (NONEXISTANT_SERVICE)
    ).is_unknown()
    assert rest.is_header_x_content_type_options_missing(
        "%s/content_type/ok" % (BAD_FORMAT_SERVICE)
    ).is_unknown()
