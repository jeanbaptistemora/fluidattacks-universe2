...
def test_cli_azure():
    ...
    testargs = [
        ...
        "-mp",
        "--azure",
        (f"{subscription_id}:{client_id}:" f"{client_secret}:{client_tenant}"),
    ]
    with patch.object(sys, "argv", testargs):
        with pytest.raises(SystemExit) as exc:
            cli.main()
