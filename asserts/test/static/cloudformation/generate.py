...
def write_template(template: troposphere.Template) -> bool:
    ...
    for extension, kwargs in (
        ("yaml", {"clean_up": True, "long_form": True}),
        ("json", {"indent": 2}),
    ):
...
cloudfront_distribution = troposphere.cloudfront.Distribution(
    ...
    DistributionConfig=troposphere.cloudfront.DistributionConfig(
        ...
        Logging=troposphere.cloudfront.Logging(
            Bucket="buckettest", IncludeCookies=False, Prefix="log_"
        ),
        Origins=[
        ...
        Restrictions=troposphere.cloudfront.Restrictions(
            GeoRestriction=troposphere.cloudfront.GeoRestriction(
                Locations=["Colombia"], RestrictionType="whitelist"
            )
        ),
    ...
    BucketEncryption=troposphere.s3.BucketEncryption(
        ServerSideEncryptionConfiguration=[
            troposphere.s3.ServerSideEncryptionRule(
                ServerSideEncryptionByDefault=troposphere.s3.ServerSideEncryptionByDefault(
                    KMSMasterKeyID="master_key", SSEAlgorithm="aws:kms"
                )
            )
        ]
    ),
...
s3_bucket_policy = troposphere.s3.BucketPolicy(
    ...
    PolicyDocument={
        "Statement": [
            {"Effect": "Allow", "Principal": "account"},
        ],
    },
)
...
cloudfront_distribution = troposphere.cloudfront.Distribution(
    ...
    DistributionConfig=troposphere.cloudfront.DistributionConfig(
        ...
        Restrictions=troposphere.cloudfront.Restrictions(
            GeoRestriction=troposphere.cloudfront.GeoRestriction(
                Locations=["Colombia"], RestrictionType="none"
            )
        ),
        ViewerCertificate=troposphere.cloudfront.ViewerCertificate(
...
param_disable = troposphere.Parameter(
    "DisableFeature", Type="String", Default=False
)
param_terminate = troposphere.Parameter(
    "TerminateInstance", Type="String", Default="terminate"
)
param_enable_public_ip = troposphere.Parameter(
    "EnablePublicIp", Type="Boolean", Default=True
)
param_backup_period = troposphere.Parameter(
    "BackupPeriod", Type="Integer", Default=0
)
...
security_group2 = troposphere.ec2.SecurityGroup(
    title="securityGroup2", GroupDescription="groupDescription2"
)
