...
in
{
  setupObservesTargetRedshiftRuntime = makeTemplate {
    ...
    template = path "/makes/products/observes/config/setup-target-redshift-runtime.sh";
  };
  setupObservesStreamerZohoCrmRuntime = makeTemplate {
    ...
    template = path "/makes/products/observes/config/setup-streamer-zoho-crm-runtime.sh";
  };
}
