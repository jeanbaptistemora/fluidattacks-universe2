module "autonomicmind_ci_cache" {
  ...
  cache_lifecycle_prefix               = "autonomicmind-ci-cache"
  cache_bucket_prefix                  = "autonomicmind-ci-cache"
...
module "autonomicmind_ci" {
  ...
  cache_bucket = {
    create = false
    policy = module.autonomicmind_ci_cache.policy_arn
    bucket = module.autonomicmind_ci_cache.bucket
  }
  ...
  gitlab_runner_registration_config = {
    registration_token = var.gitlabTokenAutonomicmind
    tag_list           = "autoscaling"
    description        = "autonomicmind-ci-${each.key}"
  ...
  runners_name                  = "autonomicmind-ci-${each.key}"
  ...
  # Tags
  environment = "makes-autonomicmind-ci-${each.key}"
  overrides = {
    name_runner_agent_instance  = "autonomicmind-ci-runner-${each.key}",
    name_docker_machine_runners = "autonomicmind-ci-worker-${each.key}",
    name_sg                     = "",
    name_iam_objects            = "",
  }
  tags = {
