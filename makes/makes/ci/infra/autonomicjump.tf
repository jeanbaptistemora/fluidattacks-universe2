module "autonomicjump_ci_cache" {
  ...
  cache_lifecycle_prefix               = "autonomicjump-ci-cache"
  cache_bucket_prefix                  = "autonomicjump-ci-cache"
...
module "autonomicjump_ci" {
  ...
  gitlab_runner_registration_config = {
    registration_token = var.gitlabTokenAutonomicjump
    tag_list           = "autoscaling"
    description        = "autonomicjump-ci-${each.key}"
  ...
  # Tags
  environment = "makes-autonomicjump-ci-${each.key}"
  overrides = {
