...
function main {
  ...
  &&  copy "${envAirsContent}" new-front/content \
  &&  copy "${envAirsContentPages}" new-front/static/images \
  &&  find new-front/static/images -type f ! -regex ".*\.\(svg\|gif\|png\|mp4\)" -delete \
  &&  copy "${envAirsContentImages}" new-front/static/images \
  &&  copy "${envAirsImages}" new-front/static/images \
