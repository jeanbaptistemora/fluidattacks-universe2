...
def test_cli_strict(test_token: str, test_group: str) -> None:
    ...
    result = runner.invoke(main, [
        '--token', test_token, '--group', test_group, '--strict',
        '--repo-path', '../'
    ])
...
def test_cli_lax(test_token: str, test_group: str) -> None:
    runner = CliRunner()
    result = runner.invoke(main, [
        '--token', test_token, '--group', test_group, '--lax', '--repo-path',
        '../'
    ])
...
def test_cli_invalid_token(test_group: str) -> None:
    runner = CliRunner()
    result = runner.invoke(main, [
        '--token', 'invalid_token', '--group', test_group, '--lax',
        '--repo-path', '../'
    ])
...
def test_cli_out_to_file(test_token: str, test_group: str) -> None:
    runner = CliRunner()
    result = runner.invoke(main, [
        '--token', test_token, '--group', test_group, '--strict', '--output',
        'test.yml', '--repo-path', '../'
    ])
