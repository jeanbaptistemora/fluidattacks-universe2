...
function main {
  ...
  main_ctx "${@}" \
    ...
    && load_commands \
    && for attribute in "${APPLICATIONS[@]}"; do
      if test "${attribute}" = "${attr}"; then
        if run_application "${attribute}" "${@:2}"; then
          echo '---' \
    ...
    && for attribute in "${PACKAGES[@]}"; do
      if test "${attribute}" = "${attr}"; then
        if build_package "${attribute}"; then
          echo '---' \
