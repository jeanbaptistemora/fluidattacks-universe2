...
sed -i 's/$CI_COMMIT_REF_SLUG/'"$CI_COMMIT_REF_SLUG"'/g' review/deploy-web.yaml review/ingress.yaml
if kubectl get ingress | grep 'review'; then
	...
	sed -n '/spec:/,/status:/p' current-ingress.yaml | head -n -1 | tail -n +5 >> review/ingress.yaml
