...
@api(risk=MEDIUM, kind=SAST)
@unknown_if(FileNotFoundError)
def has_secure_transport(path: str,
                         exclude: Optional[List[str]] =
                         None) -> tuple:
    """
    ...
    for bucket_policy, resource, template in bucket_policies:
        ...
        stmts = get_list_node_items(graph, bucket_policy, 'Statement', depth=8)
        for stmt in stmts:
            yes_secure: List = has_values(graph, stmt, 'SecureTransport',
                                          ['true', 'True', True, '1', 1],
                                          depth=12)
            no_secure: List = has_values(graph, stmt, 'SecureTransport',
...
@api(risk=MEDIUM, kind=SAST)
@unknown_if(FileNotFoundError)
def has_server_side_encryption_disabled(path: str,
                                        exclude: Optional[List[str]] =
                                        None) -> tuple:
    """
    ...
    for bucket_policy, resource, template in bucket_policies:
        ...
        stmts = get_list_node_items(graph, bucket_policy, 'Statement', depth=8)
        for stmt in stmts:
            sse: List = has_values(graph, stmt, 'x-amz-server-side-encryption',
                                   ['false', 'False', False, '0', 0],
                                   depth=19)
            for item in sse:
                if has_values(graph, stmt, 'Effect', 'Allow'):
