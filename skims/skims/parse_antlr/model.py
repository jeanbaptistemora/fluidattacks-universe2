...
def _parse_rule2(model: List[Dict[str, Any]]) -> Dict[str, Any]:
    tokens: Dict[str, Any] = {}
...
def _structure_model2(model: Dict[str, Any]) -> Dict[str, Any]:
    result: Dict[str, Any]
    ...
    if isinstance(model, dict):
        if len(model) == 1:
            ...
            if isinstance(value, list):
                if len(value) == 1:
                    # Single value list
                    result = {key: _structure_model2(value[0])}
                else:
                    # Multiple values list
                    result = {key: list(map(_structure_model2, value))}
            else:
                # Can happen?
                raise NotImplementedError()
            ...
            result = dict(zip(model, map(_structure_model2, model.values())))
    else:
        # Can happen?
        raise NotImplementedError()
...
def _structure_keys2(model: Dict[str, Any]) -> Dict[str, Any]:
    if isinstance(model, dict):
        result: Dict[str, Any] = {}
        for key, val in model.items():
            if isinstance(val, dict):
                ...
                result[key] = _parse_rule2(list(map(_structure_keys2, val)))
            else:
                # Can happen?
                raise NotImplementedError()
    else:
