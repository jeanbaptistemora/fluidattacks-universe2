...
@SHIELD_BLOCKING
def run_cfn_lambda_function_has_tracing_disabled(
    content: str, file_ext: str, path: str, template: Any
) -> Vulnerabilities:
    return cfn_lambda_function_has_tracing_disabled(
        content=content, file_ext=file_ext, path=path, template=template
    )
...
def run_tfm_trails_not_multiregion(
    ...
    return tfm_trails_not_multiregion(content=content, path=path, model=model)
        ...
        for template in load_templates_blocking(content, fmt=file_extension):
            results = (
                *results,
                ...
                run_cfn_lambda_function_has_tracing_disabled(
                    content, file_extension, path, template
                ),
            )
