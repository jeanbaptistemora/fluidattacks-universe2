data "aws_iam_policy_document" "integrates-dev-iam-policy-data" {
  statement {
    effect = "Allow"
    actions = [
      "iam:List*",
...
resource "aws_iam_policy" "integrates-dev-iam-policy" {
  description = "integrates-dev policy for iam"
  name        = "${var.user-name}-iam-policy"
  path        = "/user-provision/"
  policy      = data.aws_iam_policy_document.integrates-dev-iam-policy-data.json
}

resource "aws_iam_user_policy_attachment" "integrates-dev-attach-policy-iam" {
  user       = var.user-name
  policy_arn = aws_iam_policy.integrates-dev-iam-policy.arn
}
