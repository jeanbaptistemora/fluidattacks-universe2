data "aws_iam_policy_document" "integrates-dev-lambda-policy-data" {
  statement {
    effect  = "Allow"
    actions = ["lambda:*"]
    resources = [
...
resource "aws_iam_policy" "integrates-dev-lambda-policy" {
  description = "integrates-dev policy for lambda"
  name        = "${var.user-name}-lambda-policy"
  path        = "/user-provision/"
  policy      = data.aws_iam_policy_document.integrates-dev-lambda-policy-data.json
}

resource "aws_iam_user_policy_attachment" "integrates-dev-attach-policy-lambda" {
  user       = var.user-name
  policy_arn = aws_iam_policy.integrates-dev-lambda-policy.arn
}
