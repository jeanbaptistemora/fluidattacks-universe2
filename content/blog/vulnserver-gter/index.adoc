...
:description: Vulnserver is a VbD application focused on helping people practice exploiting skills. This post will describe the steps taken to exploit the GTER command. Besides, it will outline a technique called 'Egghunting,' used when there are space restrictions for our exploitation.
...
a double instance of that tag, which means `w00tw00t`.
...
looking for the tag `w00tw00t`.
. When the egghunter finds the tag,
it will execute anything that follows that tag.
