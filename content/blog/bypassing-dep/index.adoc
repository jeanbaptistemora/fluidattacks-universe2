...
`JMP ESP` (or any other register pointing to our shellcode) and then place
...
This will tell `mona` to find up to 10 `RETN` instrunctions. The result is:
...
`pop eax`. We also need regain control of the execution flow. This can be
done by returning back to the stack, so we can execute the last
