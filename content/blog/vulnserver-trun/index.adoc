...
Given that we have a cyclic pattern, we can check the exact 
offset of that string.
To do that, we can use `Metasploit` again:
...
As you can see, there are three register that points to our injected buffer:
`EAX`, `EBP` and `ESP`.
...
along with the continuous available space for us to inject our shellcode
...
Also, a null byte (`0x00`) on the the address of the desired instruction can
stop our attack.
...
payload gracefully slide to the start of our reverse shell.
