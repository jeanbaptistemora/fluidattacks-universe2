...
:description: Windows hosts use LLMNR and NBT-NS to perform name resolution on the local network. These protocols do not verify addresses on the network so an attacker could poison these requests in order to capture credentials. This blog post will detail how we can exploit this and obtain user hashes.
