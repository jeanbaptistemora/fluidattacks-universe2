...
the target machine. We used the following exploit:
...
to perform something more meaningful, we must know the exact offset on which
`EIP` is overwritten. Just like on any other user-space exploitation
process, we can create a cyclic pattern to find that offset. We can use
`mona` to do that:
...
Now, for illustration purposes, let's create a simple shellcode to
make `EAX = 0xdeadbeef`. We will insert it in a dynamic generated
...
link:https://en.wikipedia.org/wiki/Protection_ring[Protection rings]. Read
the article to details. In the context of this article, the relevant
information is that Windows uses only 2 Current Privilege Levels (`CPL`):
0 and 3. `CPL` levels are also referred as `rings`. `CPL0` or `ring-0` is
where the kernel is executed and `CPL3` or `ring-3` is where user mode
...
Technically, `SMEP` is the 20th bit of the `CR4` control register:
...
`ring-3`? link:../bypassing-dep/[ROP] comes to the rescue! At `ring-0` `ROP`
is often referred as `kROP` because we will reuse instructions located at
kernel level. We then need to execute a `kROP` chain and change the value of
`CR4`. With that, we should be able to make `EAX = 0xdeadbeef`.
...
and then pop 8 bytes from the stack:
...
We also bypassed `SMEP` protection using a `kROP` gadget and defeated `kASRL`
