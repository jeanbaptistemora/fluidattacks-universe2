...
:description: Server Side Request Forgery, is a vulnerability that lets the hacker to perform requests outside the scope of the application. Here we will see why this happens, how it works, how we can exploit it and what are the best ways to protect against this attack.
...
Many web applications requests outside services
for data, configurations, updates, among others.
This is beneficial for the developers and maintainers
because it keeps a separation of duties in their infrastructure,
one for managing the view and another for the data.
When it is done right this applications are easier to maintain
and to add features,
...
an attacker could create requests
from the vulnerable server to the internet/intranet.
Typically the vulnerable server have a functionality
that reads data from an URL, publish data to an URL
or import data from an URL.
An attacker could abuse this functionality
to read or update internal resources,
also could bypass access controls
like firewalls that prevent the attackers to access them directly.

In a normal use case the vulnerable application
works like this:

.normal use case
image::normal-case.png[normal-case]

. The user request information from the Web Server
...
. The requests passes through the company firewall
bypassing its measures
. The admin server responds with the data requested to the attacker
...
Then using `Dirbuster` we can search for wed directories on the web server.
...
As we can see, there is an admin site that we do not have access,
...
Lets change it to `https://owasp.org/`:
...
Now we have several options to work here.

Reflected XSS::

Lets create an `SVG` image in our kali machine
with an `XSS` payload
...
Also `dict://` URL schema to connect to a server and send data:
...
Because the server uses `cURL` there are some URL schemas
that this library do not support,
like `ssh22`, `expect`, among others.
For more information
and payloads you can go link:https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/Server%20Side%20Request%20Forgery[here]
...
It could be in the form of validate the domain name of the target host
using a whitelist.
With this if the attacker tries to access more resources
it will be impossible for him.

Also if it is possible avoid sending URLs
...
This will prevent the access to other resources in the network
