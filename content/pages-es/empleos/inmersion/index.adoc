...
. Realizar *cinco (5)* rondas de retos.

.. Una ronda de retos consiste en realizar en estricto orden:

... *un (1)* +MR+ de +systems-hacking+ aceptado en la rama master del
link:https://gitlab.com/fluidattacks/writeups[repo writeups],

... *un (1)* +MR+ de +ctf-hacking+ aceptado en la rama master del
link:https://gitlab.com/autonomicmind/training/[repo training], y

... *un (1)*  +MR+ de +programación+ aceptado en la rama master del
link:https://gitlab.com/autonomicmind/training/[repo training].


.. No deben hacerse +MRs+ consecutivos con soluciones de un mismo tipo.
.. No debe cambiarse el orden de entrenamiento y envio de soluciones.
.. No deben saltarse ningun tipo de retos.

. Luego de hacer *cinco (5)* rondas de retos,
realizar *un (1)* artículo para el blog.

. Repetir los pasos anteriores hasta completar los
[button]#link:../inmersion/#objetivos[objetivos]#.

.. En resumen:
rondas de [red]#(syst ctfh prog )\{5\}blog# hasta completar los objetivos.

. Para los retos de programación tener en cuenta:
...
... Lee sobre la temática que no dominas,
(grafos, arboles, aritmética modular, etc).

... Aborda el problema desde enfoques diferentes,
(Recursión, Heurística, Programación Dinámica, Codicia, Divide y Vencerás, etc).

... Persevera! nos interesan programadores disciplinados.

.. Si se alcanza el punto en que definitivamente no logras avanzar:
