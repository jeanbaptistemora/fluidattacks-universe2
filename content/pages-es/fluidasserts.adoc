...
*FLUIDAsserts* es un motor de automatización de *cierre* de hallazgos de seguridad sobre ambientes de ejecución +(DAST)+.
...
fluidasserts:
  script:
    ...
    - docker run -v "$PWD"/fusion:/code -e FLUIDASSERTS_STRICT=$STRICT fluid-docker.jfrog.io/<cliente>:<app>
  artifacts:
