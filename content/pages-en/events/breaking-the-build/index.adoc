...
:category: events
:description: This page aims to inform the customer about the different talks offered by Fluid Attacks. In the Breaking the Build conference we talk about our SecDevOps habits that allow us to keep improving and changing everyday, and the first steps to implement these habits on your company.
:keywords: Fluid Attacks, SecDevOps, Security, Development, Tecnology, CI-CD.
...
ignoring the people's main reason to assist to this kind of events.
...
and in this case, new questions will arise, such as
*does this really work?* or *how can I apply it to my company?*
...
the concepts and techniques seen in
...
The tools used,
the logs that allow to identify issues,
...
In order to understand how everything happens
...
through a *microchanges* methodology
(instead of big changes every +3+ weeks or more).

* Migrate and manage all the infrastructure as a versioned source code,
...
+*pipeline as code*+,
in a way that can be easily configured
and modified for all kind of tests
(avoiding graphical inteface limitations
...
using +*pre-commit*+ for checking source code.
...
The event date must be scheduled by mutual agreement
