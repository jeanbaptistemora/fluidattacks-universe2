...
:category: products
:description: In this page we present the products offered by FLUID. FLUIDAsserts is an engine to automate the closing of security findings over execution environments, which can be implemented in a CI pipeline to determine if a security issue persists in the application.
:keywords: FLUID, Products, FLUIDAsserts, Ethical Hacking, Pentesting, Security.

= FLUIDAsserts

== 1. Description

+FLUIDAsserts+ is an engine to automate the *closing* of security findings
...
If you have a subscribed application to our *Concurrent Security Tests* service
which includes the use of +FLUIDAsserts+,
you may add it to your Continuous Integration pipeline.
To achieve this, follow the steps listed below:

* Add the environmental variables +USER+, +PASS+, +ORG+ and +APP+.
...
.Example of environmental variables in Gitlab CI
...
.ex1_close.py: Verify already solved SQL injection
[source, python, linenum]
----
from fluidasserts.service import http
...
.ex2_open.py: Verify if XSS still present
[source, python, linenum]
----
from fluidasserts.service import http
...
.ex2_close.py: Verify already solved XSS
[source, python, linenum]
----
from fluidasserts.service import http
