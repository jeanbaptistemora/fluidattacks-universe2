...
in order to upload it to our +FLUIDAttacks+ youtube channel.
...
is available in the +FLUIDAttacks+ website.
...
with the community and you are not part of +FLUIDAttacks+,
...
. +FLUIDAttacks+ reserves the rights of admission
of all documents sent in.

. We perform a non-substantive review of the document.
+FLUIDAttacks+ doesn’t evaluate if we agree or not with the author’s opinion but
...
the author *transfers* the copyrights of said document to +FLUIDAttacks+;
If necessary,
