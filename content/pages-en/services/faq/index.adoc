...
*How does Fluid Attacks stay on top of new techniques and changes?*::
  ...
  Either through link:../../blog[blog entries] or link:https://gitlab.com/fluidattacks/writeups[hacking challenges]
...
*What options for retesting are available?*::
 ...
 link:../continuous-hacking/[Continuous Hacking] includes infinite retest
...
*while the test is taking place*::
 ...
 We recommend to include only the +staging+ phase in the scope,
 however, many customers decide to include +production+ stage in the tests.
 It is unusual for us to take down environments,
 because when we foresee a possible breakpoint,
 we ask the customer for an special environment for such test.
