...
+Semmle+ does not tell much about the process,
since obviously the magic behind their product must be there,
but let's try to infer from Linton's ideas.
...
Linton shows the link:https://www2.eecs.berkeley.edu/Pubs/TechRpts/1983/CSD-83-164.pdf#page=31[actual tables]
and relations cut to the
most relevant parts.
I'll try to simplify that further
by reducing it to
an informal entity-relation diagram.
...
The most basic entity here is the +statement+,
which is basically every line of code.
It can consist of a conditional,
an assigment or maybe a function call.
...
like the logical programming language +Prolog+.
However, this would add too much overhead,
so a simpler sublanguage called +Datalog+
...
Analogous +SQL+ queries would take many more lines
comprising several selects, groupings and joins.
...
This last item is what interests us most.
+Semmle+ maintains a public queries link:https://github.com/lgtmhq/lgtm-queries[repository],
a website with general 'rules' that should be
...
here is the query to find +Heartbleed+-like vulnerabilities:
...
The +Apache Struts+ vulnerability +CVE-2017-9805+,
related but not to be confused with +CVE-2017-5638+,
the one that was exploited in the +equifax+ breach,
...
the free-for-open-source-projects kid sibling to +Semmle+.
