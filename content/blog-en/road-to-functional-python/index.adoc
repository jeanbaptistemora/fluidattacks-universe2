...
when you think you're ready, come back to the road,
...
such as +itertools+ or +functools+ (of which we will go into details later)
...
def greater_than_seven (x):
  if x > 7:
    return true
  else:
    return false

filter(greater_than_seven, integer_list) # [8, 9]
----
...
One of the most prominent is +Itertools+,
...
""" Partial, allows to partially execute a function, generating a new function with fewer input arguments """
def multiply(a,b):
        return a * b
