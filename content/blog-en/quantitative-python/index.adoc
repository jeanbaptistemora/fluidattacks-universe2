...
.Data as imported into Jupyter like in our link:../monetizing-vulnerabilities/#input-data[Monte Carlo article].
...
.Simple loss exceedance curve like in our link:../monetizing-vulnerabilities/#lec-simple[Monte Carlo article].
...
as described in link:../monetizing-vulnerabilities/[the original article]
...
.Loss exceedance curves like in our link:../monetizing-vulnerabilities/#lec-all[Monte Carlo article].
...
That is, let us randomize the table in <<inputs, figure 1>> above.
We can skip the importing and the column with the events names,
since we don't use them.
We simply generate a column of random numbers between appropriate boundaries:
0 and 0.1 for the probabilities,
0 and a million for the lower `CI` bounds,
a million and a billion for the upper `CI` bounds.
Finally we glue them together in a `Pandas` `DataFrame`
so they are usable to our previous functions:
...
Next we run Monte-Carlo on such a simulated `DataFrame`
of events, once for each day of a fictitious month,
compute the `VaR` and `tVaR` for each day
and observe how they evolve:

.Fabricated VaR monitoring example like in our link:../para-bellum/#var-monitor[VaR article].
image::monitor-var-time.png[Evolution of daily VaR in month]

Since this is a made-up example and
...
----
include::/web/content/blog-en/quantitative-python/quant.py[]
----
