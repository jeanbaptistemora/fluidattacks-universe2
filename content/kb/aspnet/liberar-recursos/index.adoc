:slug: kb/aspnet/liberar-recursos/
:eth: no
:category: aspnet
:kb: yes
...
normalmente se recurre a las sentencias _try_ y _catch_.
Se coloca el código que puede producir
una excepción dentro de la sentencia _try_ 
y el código a ejecutar cuando se presente en la sentencia _catch_.
...
se recurre a la sentencia _finally_. 
Al enviar un bloque de código a _finally_
es posible limpiar todos los recursos
localizados en una sentencia _try_.
La ventaja de la sentencia _finally_ es
que permite ser ejecutada cada vez 
que una excepción ocurre en el bloque _try_. 
Si se combinan estas dos sentencias, 
en conjunto con la sentencia _catch_,
...
dentro del bloque _try_, en este caso se utilizará 
...
. El bloque _catch_ capturará la excepción 
y ejecutará el código definido por el usuario 
en caso de presentarse dicha excepción. 
...
se debe realizar dentro de la cláusula _finally_. 
Ésto con el propósito de garantizar 
que si ocurre algún evento excepcional 
dentro de la cláusula _try_, 
...
. https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/try-catch-finally[Microsoft, Try- Catch- Finally]

. https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/try-finally[Microsoft, Try-Finally]