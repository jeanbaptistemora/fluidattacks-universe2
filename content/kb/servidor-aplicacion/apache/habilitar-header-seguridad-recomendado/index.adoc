:slug: kb/servidor-aplicacion/apache/habilitar-header-seguridad-recomendado
:eth: no
:category: apache
:kb: yes

= Habilitar los Headers de seguridad recomendados
...
Header always set X-Content-Security-Policy "default-src 'self'; img-src 'self'; 
  style-src 'self' 'unsafe-inline'; font-src 'self'; script-src 'self' 'unsafe-inline'; connect-src
   'self';" 
