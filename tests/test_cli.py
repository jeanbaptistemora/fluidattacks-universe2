...
def test_resources(relocate):
    ...
    test_clone = runner.invoke(cli, f'resources --clone {REPOSITORY}'.split())
    assert test_clone.exit_code == 0

    test_does_subs_exist = runner.invoke(
        cli, f'resources --does-subs-exist {REPOSITORY}'.split())
    assert test_does_subs_exist.exit_code == 0

    test_mailmap = runner.invoke(cli,
                                 f'resources --mailmap {REPOSITORY}'.split())

    assert test_mailmap.exit_code == 0
    assert 'Mailmap sorted' in test_mailmap.output

    test_read = runner.invoke(cli, f'resources --read {REPOSITORY}'.split())
...
def test_forces(relocate):
    ...
    test_check_sync = runner.invoke(
        cli, f'forces --check-sync all {REPOSITORY}'.split())
    assert test_check_sync.exit_code == 0

    test_lint_exps = runner.invoke(
        cli, f'forces --lint-exps all {REPOSITORY}'.split())
    assert test_lint_exps.exit_code == 0

    test_run_static = runner.invoke(
        cli, f'forces --run-exps --static all {REPOSITORY}'.split())
    ...
    test_run_dynamic = runner.invoke(
        cli, f'forces --run-exps --dynamic all {REPOSITORY}'.split())
    ...
    plain_text = (f'subscriptions/{REPOSITORY}/break-build/'
                  'dynamic/resources/plaintext.yml')
    test_decrypt = runner.invoke(cli,
                                 f'forces --decrypt {REPOSITORY}'.split())
    ...
    test_encrypt = runner.invoke(cli,
                                 f'forces --encrypt {REPOSITORY}'.split())
...
def test_integrates(relocate):
    ...
    test_get_dict = runner.invoke(
        cli, f'integrates --get-static-dict {REPOSITORY}'.split())
