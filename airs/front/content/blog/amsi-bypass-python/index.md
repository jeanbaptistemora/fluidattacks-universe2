...
WinDbg.
However, it is not common to have debugger in a victim machine
when performing a [red teaming](../../solutions/red-teaming)
operation.

There's lots of methods around that weaponize the memory
...
AMSI-backed EDR signatures. Let's look at another
...
We can't use the same strategy for patching AMSI if
...
So, as a general strategy we need to do the
following:
...
`amsi.dll` is there, but also a bunch
...
process memory. Let's check that with the following code:
...
prologue opcodes and will increment the
...
time with a whole different process using
cross-process memory patching.

This is the final script. I rearranged it
...
can see, AMSI can be easily bypassed using
