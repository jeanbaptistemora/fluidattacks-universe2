:page-slug: hevd-smep-bypass/
:page-date: 2020-09-18
:page-category: attacks
:page-subtitle: Bypassing OS protections
:page-tags: osee, training, exploit, windows, kernel, hevd
:page-image: https://res.cloudinary.com/fluid-attacks/image/upload/v1620330914/blog/hevd-smep-bypass/cover_iwwedx.webp
:page-alt: Photo by Michael Dziedzic on Unsplash
:page-description: In this article we will defeat some protections using several techniques for exploting HackSys Extremely Vulnerable Driver
...
current kernel base address.

However, other protections come to scene when trying to exploit at kernel
level, like `DEP`, `SMEP`, `CFG`, etc. We will surely come across with some
of them later. Stay tuned.
...
to perform something more interesting, we must start by locating the exact
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330915/blog/hevd-smep-bypass/mona1_bhfpei.gif[Mona Cyclic Pattern]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330917/blog/hevd-smep-bypass/mona2_nljhdo.gif[Mona Cyclic Pattern]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330917/blog/hevd-smep-bypass/smep1_zdy5uw.gif[SMEP in action]
...
because `HEVD` is executing at `ring-0` and after overwriting `EIP`, it was
trying to run the instructions in our shellcode which was allocated at
`ring-3`.

Technically, `SMEP` is nothing but a bit in a CPU control register,
specifically the 20th bit of the `CR4` control register:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330911/blog/hevd-smep-bypass/cr4_wzdcnp.webp[CR4 register]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330912/blog/hevd-smep-bypass/cr42_abunfu.webp[CR4 register]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330913/blog/hevd-smep-bypass/cr4-rop_qsyqgw.webp[CR4 ROP]

Now, we need to calculate the offset of that `ROP` gadget from the start
of the `nt` module:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330911/blog/hevd-smep-bypass/offset1_ztkull.webp[CR4 ROP Offset]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330912/blog/hevd-smep-bypass/popeax-rop_fuafnn.webp[POP EAX ROP]

And the offset from the start of the `nt` module is `0002bbef`:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330912/blog/hevd-smep-bypass/offset2_tzgwnp.webp[POP EAX Offset]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330911/blog/hevd-smep-bypass/rop-padding_b9rtmd.webp[ROP Padding]
...
`SMEP`. However, we need to deal with kernel `ASLR`. As I mentioned before,
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330912/blog/hevd-smep-bypass/leak1_jtbpi1.webp[Kernel Base Address]

As you can see, it matches perfectly to the address reported by `WinDBG`:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330911/blog/hevd-smep-bypass/leak2_iwvv7t.webp[Kernel Base Address]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330911/blog/hevd-smep-bypass/success1_ji7qih.gif[Success]

And this is the content of the `CR4` register:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620330912/blog/hevd-smep-bypass/smep-disabled_fsvseg.webp[Success]
...
We also bypassed `SMEP` protection using a `kROP` chain and defeated `kASLR`
by leaking the kernel base address from `ring-3`.
However, we have still have to get a privileged shell on this system, which
will be covered in the link:../hevd-privilege-escalation/[next article].
