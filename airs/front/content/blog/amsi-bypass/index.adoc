...
:page-subtitle: Bypassing AMSI using memory patching
...
in the middle of a link:../../pages/solutions/red-teaming[Red Team] engagement.
...
Technically, if an application was built with `AMSI`, `amsi.dll` will become
...
Success! Now we can use `PowerView`, `Invoke-Mimikatz` or any other
 link:../../pages/solutions/red-teaming[Red Team] tools!
