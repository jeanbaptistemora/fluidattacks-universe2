:page-slug: vulnserver-gter/
:page-date: 2020-06-11
:page-category: attacks
:page-subtitle: Hunting eggs
:page-tags: osce, vulnserver, training, exploit
:page-image: https://res.cloudinary.com/fluid-attacks/image/upload/v1620331147/blog/vulnserver-gter/cover_prsww3.webp
:page-alt: Photo by Annie Spratt on Unsplash
:page-description: This post will describe the steps taken to exploit the Vulnserver GTER command, using a technique called 'Egghunting.'
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331147/blog/vulnserver-gter/vulnserver1_qvelza.webp[vulnserver]

We can check the behavior of the `GTER` command by issuing the suggested
syntax.

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331143/blog/vulnserver-gter/gter1_bow4kj.webp[GTER command syntax]
...
we discovered the syntax needed for the `GTER` command.
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331146/blog/vulnserver-gter/fuzz1_eahyra.gif[Running spike]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/wireshark1_ohyi15.webp[Wireshark capture]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331144/blog/vulnserver-gter/gter2_axxim1.gif[Exploit: take 1]

Look at that! We replicated the crash!

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331146/blog/vulnserver-gter/crash1_tnurkm.webp[Controlling EIP]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/pattern1_rxyoot.webp[Cyclic pattern]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/offset1_uk8di2.webp[Exploit: take 2]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/offset2_bv13e7.webp[Correct offset to EIP]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/mona1_ooj8ce.webp[JMP ESP]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/jmp1_ceeuzf.gif[Performing the JMP ESP]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331145/blog/vulnserver-gter/jmp1_vnn3ms.webp[Location of our execution flow]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331146/blog/vulnserver-gter/jmp2_yj95iy.webp[Start of buffer]

So we need to do a relative reverse jump. Using the debugger, we can
easily calculate the offset:

image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331144/blog/vulnserver-gter/jmp3_gyybou.gif[Relative backward jump]
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331144/blog/vulnserver-gter/jmp4_ebtqur.gif[Jumped back]
...
It's a special shellcode because its only mission is to search the entire
process memory for the location of the desired code,
...
. The reverse jump will be executed
...
it will execute anything that follows, in our case, a bind shellcode.
...
to send the shellcode over! That will store the shellcode on the process
memory where our egghunter will be able to find it.
...
image::https://res.cloudinary.com/fluid-attacks/image/upload/v1620331147/blog/vulnserver-gter/shell1_whl7kp.gif[Success]
