...
def filter_data_greater_than(
    ...
    filtered_records = api_client.elements_greater_than(target_id, data)
    file = tempfile.NamedTemporaryFile(mode="w+")
    extractor.emit(stream, filtered_records, file)
    return PageData(id=dpage.id, file=file, minor_item_id=dpage.minor_item_id)
...
async def stream_resource_page(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if m_id is not None:
            ...
            return PageData(resource, file=out_file, minor_item_id=m_id)
        out_file.close()
        return None
