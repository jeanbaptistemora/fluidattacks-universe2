...
def test_target_dates_month_start():
    # Arrange
    ...
    # Assert
    assert any(target_dates.previous_ranges.contains(today)) == False
    assert today in target_dates.actual_range


@pytest.mark.freeze_time('2017-05-15')
def test_target_dates_middle_date():
    # Arrange
    today = Timestamp(datetime.date.today())
    a_year_ago = today - DateOffset(days=365)
    # Act
...
def test_target_dates_month_end():
    # Arrange
    today = Timestamp(datetime.date.today())
    a_year_ago = today - DateOffset(days=365)
    # Act
    target_dates = planner.target_dates(a_year_ago)
    # Assert
    assert any(target_dates.previous_ranges.contains(today)) == True
