...
def test_tabledraft_factory() -> None:
    ...
    test_table_id = TableID(test_schema, table_name="super_table")
    # Act
...
def test_tid_factory_rschema_to_tid() -> None:
    ...
    test_rschema = RedshiftSchema(
        fields=frozenset(), schema_name="the_schema", table_name="super_table"
    )
    # Act
    result = factory.rschema_to_tid(test_rschema)
    # Assert
    expected = TableID(schema=test_schema, table_name="super_table")
    assert result == expected
...
def test_tid_factory_srecord_to_tid() -> None:
    ...
    # Assert
    expected = TableID(schema=test_schema, table_name="table1")
    assert result == expected
