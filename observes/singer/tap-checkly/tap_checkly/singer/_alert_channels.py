...
def _core_encoder_fx() -> SingerEncoder[AlertChannelObj]:
    _mapper: Dict[str, EncodeItem[AlertChannelObj]] = {
        "alert_ch_id": EncodeItem.new(
            lambda x: x.id_obj.id_int, Property(_int_type, True, False)
        ),
        "alert_type": EncodeItem.new(
            lambda x: x.obj.alert_type, Property(_str_type, False, False)
        ),
        "send_recovery": EncodeItem.new(
            lambda x: x.obj.send_recovery, Property(_bool_type, False, False)
        ),
        "send_failure": EncodeItem.new(
            lambda x: x.obj.send_failure, Property(_bool_type, False, False)
        ),
        "send_degraded": EncodeItem.new(
            lambda x: x.obj.send_degraded, Property(_bool_type, False, False)
        ),
        "ssl_expiry": EncodeItem.new(
            lambda x: x.obj.ssl_expiry, Property(_bool_type, False, False)
        ),
