...
def _calc_cvss2_temporal(
        ...
        basescore: Union[float, Decimal]) -> Decimal:
    """Calculate cvss v2 temporal attribute."""
    temporal = Decimal(
        float(basescore) * severity['exploitability'] *
...
def _calc_cvss2_basescore(
    ...
    exploitabilty = (
        parameters['exploitability_factor'] * severity['accessComplexity'] *
...
def _calc_cvss2_environment(
        severity: Dict[str, float],
        parameters: Dict[str, float]) -> Decimal:
