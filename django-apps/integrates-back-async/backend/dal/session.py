...
def get_all_logged_users() -> List[Dict[str, str]]:
    ...
    sessions_values_usernames = [
        {json.loads(':'.join(base64.b64decode(
            REDIS_CLIENT.get(key)).decode('utf-8').split(':')[1:])).get('username'): key}
...
def get_previous_session(user_mail: str, session_key: str) -> Optional[str]:
    """ checks if exists other active session with the same user_mail and if so returns it"""
    all_active_sessions = get_all_logged_users()
    old_session_key = [
        item[1] for item in
        [list(session_value.items())[0] for session_value in all_active_sessions]
        if item[0] == user_mail and item[1] != 'fi_session:{}'.format(session_key)]
