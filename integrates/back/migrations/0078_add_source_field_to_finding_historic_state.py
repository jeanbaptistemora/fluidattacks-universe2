...
async def add_source_field_to_historic_state(
    ...
    old_finding_historic_state = cast(
        Historic, finding.get("historic_state", [])
    )
    ...
    for finding_state_info in finding_historic_state:
        if not finding_state_info.get("source"):
            to_update = True
            if finding_state_info.get("analyst", "") in skims_analysts and (
                ...
                if vulns is None:
                    vulns = await dynamodb_ops.query(
                        VULNERABILITY_TABLE, vuln_query_attrs
                    )
                    for vuln in vulns:
                        vuln_historic_state = cast(
                            Historic, vuln["historic_state"]
                        )
    ...
    if to_update:
        success = await findings_dal.update(
            finding_id, {"historic_state": finding_historic_state}
        )
