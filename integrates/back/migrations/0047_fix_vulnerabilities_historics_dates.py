...
def get_first_historic_attribute_date(
    historic_list: List[Dict[str, str]], attribute: str
) -> str:
    if historic_list:
...
def print_test_stage(  # pylint: disable=too-many-arguments
    finding_id: str,
    vuln_id: str,
...
# historics dates
async def fix_vuln_historics(  # pylint: disable=too-many-locals,too-many-statements # noqa: MC0001
    vulnerability: Vulnerability, fix_date: str
) -> None:
    new_state = []
    new_treatment = []
    ...
    first_open_state = get_first_historic_attribute_date(
        historic_state, "state"
    )
    first_new_treatment = get_first_historic_attribute_date(
        historic_treatment, "treatment"
    )
    ...
    if handle_historic_default(first_open_state, first_new_treatment):
        ...
        # date of the fix date
            ...
            new_treatment.append(
                {"date": f"{fix_date} 00:00:00", "treatment": "NEW"}
            )
    ...
    # Handle when the first treatment is after the first open
        ...
        treatment_is_default
        or first_new_treatment > first_open_state
        ...
        # historics
            ...
            new_treatment.append(
                {"date": f"{first_open_state}", "treatment": "NEW"}
            )
    ...
    if treatment_is_changed:
        if STAGE == "apply":
            ...
            await vulns_dal.update(
                finding_id, vuln_id, {"historic_treatment": new_treatment}
            )
        else:
            print_test_stage(
                finding_id,
    ...
    if state_is_changed:
        if STAGE == "apply":
            ...
            print(f"finding_id: {finding_id}")
            await vulns_dal.update(
                finding_id, vuln_id, {"historic_state": new_state}
            )
        else:
            print_test_stage(
                finding_id,
...
async def fix_vulns(finding: str) -> None:
    vulneabilities = await vulns_domain.list_vulnerabilities_async([finding])
    dates = finding_utils.get_tracking_dates(vulneabilities)
    filter_dates = [date for date in dates if date > "2000-01-01 00:00:00"]
    filter_dates.sort()
    if not filter_dates:
        finding_data = await findings_domain.get_finding(finding)
        historic_state = cast(
            List[Dict[str, str]], finding_data.get("historic_state", [])
        )
        if historic_state:
...
async def fix_vulnerabilities_historics(groups: List[str]) -> None:
    ...
    await collect(
        [await in_thread(fix_vulns, finding) for finding in findings_ids]
    )
