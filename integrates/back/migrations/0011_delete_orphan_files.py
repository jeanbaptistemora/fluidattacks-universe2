...
def clean_finding_evidences(group_name: str) -> None:
    findings = findings_domain.list_drafts(
        group_name, include_deleted=True
    ) + findings_domain.list_findings(
        get_new_context(), group_name, include_deleted=True
    )
    ...
    for finding_id in findings:
        ...
        for file_name in findings_dal.search_evidence(evidence_prefix):
            if (
                ...
                and file_name != f"{group_name}/{finding_id}/"
            ):
                if STAGE == "test":
