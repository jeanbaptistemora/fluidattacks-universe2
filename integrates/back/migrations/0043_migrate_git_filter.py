...
async def update_filter(group_name: str, root: Dict[str, Any]) -> None:
    states_to_update = [
        {**state, "filter": format_new_filter(state.get("filter"))}
    ...
    if STAGE == "test":
        ...
        await roots_dal.update_legacy(
            group_name, root["sk"], {"historic_state": states_to_update}
        )
...
async def main() -> None:
    ...
    for group_name in groups:
        ...
        await collect(update_filter(group_name, root) for root in roots)
