...
def replace_unsafe_strings(original_string: str, field: str) -> str:
    ...
    if (
        safe_string != original_string and not
        re.match(STRING_REGEX, safe_string)
    ):
...
if __name__ == '__main__':
    ...
    for finding_to_process in get_all_findings():
        if execute_flag:
            if (
                finding_to_process.get('vulnerability_new', '') or
                finding_to_process.get('effect_solution_new', '')
            ):
