...
async def _validate_acceptance_days(
    ...
    is_valid_acceptance_date = await in_thread(
        finding_utils.validate_acceptance_date,
        values
    )
...
def get_treatment_change(
    vulnerability: Dict[str, Finding],
    min_date: Datetime
...
async def handle_vuln_acceptation(
    *,
    ...
    if (
        historic_treatment and
        new_treatments[-1].get('acceptance_status') == 'APPROVED' and
        'treatment_manager' in historic_treatment[-1]
    ):
...
async def handle_vulns_acceptation(
    *,
    ...
    if (
        not vulnerabilities or
        len([
            vuln
            for vuln in vulnerabilities
            if (
                vuln['id'] in vuln_ids and
...
async def send_treatment_change_mail(
    ...
    changes = list(
        filter(
            None,
            [get_treatment_change(vuln, min_date) for vuln in vulns]
        )
    )
