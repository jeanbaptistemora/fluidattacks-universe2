...
===== {{requisite_title}}
    {% for req in finding.recommendation.split("\n") %}
        - {{req + "\n"}}
    {% endfor %}
==== {{where_title}}
{% if finding.grouped_ports_vulnerablities or finding.grouped_lines_vulnerablities or finding.grouped_inputs_vulnerablities %}
===== {{ports}}
|===
|{{'Host'}}|{{port}}
...
{% for line in finding.grouped_lines_vulnerablities %}
...
|===
|{{'URL'}}|{{field}}
{% for input in finding.grouped_inputs_vulnerablities %}
