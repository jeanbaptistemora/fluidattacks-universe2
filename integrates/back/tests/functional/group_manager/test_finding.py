...
async def test_finding() -> None:
    ...
    assert result["data"]["finding"][
        "openVulnerabilities"
    ] == expected_output.get("open_vulnerabilities")
    assert result["data"]["finding"][
        "closedVulnerabilities"
    ] == expected_output.get("closed_vulnerabilities")
    assert result["data"]["finding"]["tracking"] == expected_output.get(
        "tracking"
    )
    assert result["data"]["finding"]["records"] == expected_output.get(
        "records"
    )
    assert result["data"]["finding"]["severity"] == expected_output.get(
        "severity"
    )
    assert result["data"]["finding"]["cvssVersion"] == expected_output.get(
        "cvss_version"
    )
    assert result["data"]["finding"]["evidence"] == expected_output.get(
        "evidence"
    )
    assert result["data"]["finding"]["state"] == expected_output.get("state")
    assert result["data"]["finding"]["title"] == expected_output.get("title")
    assert result["data"]["finding"]["scenario"] == expected_output.get(
        "scenario"
    )
    assert result["data"]["finding"]["actor"] == expected_output.get("actor")
    assert result["data"]["finding"]["description"] == expected_output.get(
        "description"
    )
    assert result["data"]["finding"]["requirements"] == expected_output.get(
        "requirements"
    )
    assert result["data"]["finding"][
        "attackVectorDesc"
    ] == expected_output.get("attack_vector_desc")
    assert result["data"]["finding"]["threat"] == expected_output.get("threat")
    assert result["data"]["finding"]["recommendation"] == expected_output.get(
        "recommendation"
    )
    assert result["data"]["finding"]["affectedSystems"] == expected_output.get(
        "affected_systems"
    )
    assert result["data"]["finding"][
        "compromisedAttributes"
    ] == expected_output.get("compromised_attributes")
    assert result["data"]["finding"][
        "compromisedRecords"
    ...
    assert result["data"]["finding"]["remediated"] == expected_output.get(
        "remediated"
    )
    assert result["data"]["finding"]["type"] == expected_output.get("type")
    assert result["data"]["finding"]["isExploitable"] == expected_output.get(
        "is_exploitable"
    )
    assert result["data"]["finding"]["severityScore"] == expected_output.get(
        "severity_score"
    )
    assert result["data"]["finding"]["reportDate"] == expected_output.get(
        "report_date"
    )
    assert result["data"]["finding"]["currentState"] == expected_output.get(
        "current_state"
    )
    assert result["data"]["finding"]["newRemediated"] == expected_output.get(
        "new_remediated"
    )
    assert result["data"]["finding"]["verified"] == expected_output.get(
        "verified"
    )
    assert result["data"]["finding"]["vulnerabilities"] == expected_output.get(
        "vulnerabilities"
    )
    assert result["data"]["finding"]["portsVulns"] == expected_output.get(
        "ports_vulns"
    )
    assert result["data"]["finding"]["inputsVulns"] == expected_output.get(
        "inputs_vulns"
    )
    assert result["data"]["finding"]["linesVulns"] == expected_output.get(
        "lines_vulns"
    )
    assert result["data"]["finding"]["analyst"] == expected_output.get(
        "analyst"
    )
    assert result["data"]["finding"]["observations"] == expected_output.get(
        "observations"
    )
    ...
    context = get_new_context()
    expected_output = {
        "consulting": {"content": consult_content},
        "observations": [
            {"content": consult_content, "email": "unittest2@fluidattacks.com"}
        ],
    }
    query = f"""{{
        finding(identifier: "{finding_id}"){{
    ...
    assert "errors" not in result
    assert (
        expected_output.get("consulting")
        not in result["data"]["finding"]["consulting"]
    )
    assert result["data"]["finding"]["observations"] == expected_output.get(
        "observations"
    )
