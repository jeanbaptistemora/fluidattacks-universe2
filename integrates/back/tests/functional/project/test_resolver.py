...
async def test_get_project(populate: bool, email: str) -> None:
    assert populate
    ...
    assert finding in [
        finding["id"] for finding in result["data"]["project"]["findings"]
    ]
    assert event in [
        event["id"] for event in result["data"]["project"]["events"]
    ]
