...
async def _test_forces() -> None:
    ...
    assert (
        result["data"]["forcesExecution"]["gitBranch"] == "unable to retrieve"
    )
    assert (
        result["data"]["forcesExecution"]["gitCommit"] == "unable to retrieve"
    )
    assert (
        result["data"]["forcesExecution"]["gitOrigin"] == "unable to retrieve"
    )
    ...
    assert (
        result["data"]["forcesExecution"]["vulnerabilities"][
            "numOfAcceptedVulnerabilities"
        ]
        == 0
    )
    ...
    assert (
        result["data"]["forcesExecution"]["vulnerabilities"]["accepted"] == []
    )
    assert (
        len(result["data"]["forcesExecution"]["vulnerabilities"]["closed"])
        == 8
    )
    assert (
        len(result["data"]["forcesExecution"]["vulnerabilities"]["open"]) == 32
    )
