...
async def generate_one(
    group: str, context: Any, min_date: Optional[date]
) -> Decimal:
    return await get_mean_remediate(context, group.lower(), min_date)


async def get_many_groups(
    groups: Iterable[str], context: Any, min_date: Optional[date] = None
) -> Decimal:
...
async def generate_all() -> None:
    ...
    async for group in utils.iterate_groups():
        utils.json_dump(
            document=format_data(
                mean_remediate=await generate_one(group, context, None),
            ),
            entity="group",
            subject=group,
        )
    ...
    async for org_id, _, org_groups in (
        ...
        utils.json_dump(
            document=format_data(
                mean_remediate=await get_many_groups(org_groups, context),
            ),
            entity="organization",
            subject=org_id,
        )
