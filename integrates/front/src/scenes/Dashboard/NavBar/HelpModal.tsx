...
const HelpModal: FC<IHelpModalProps> = ({
  onClose,
  open,
}: Readonly<IHelpModalProps>): JSX.Element => {
    ...
    <StrictMode>
      <Modal onClose={onClose} open={open} title={t("navbar.help.support")}>
        <Row cols={3}>
          <Col>
            <Card>
              <Button icon={faComment} onClick={toggleZendesk} size={"lg"}>
                {t("navbar.help.chat")}
              </Button>
              <Text>
          ...
          {match === null || data === undefined ? (
            ...
            <Col>
              <Card>
                <Button icon={faHeadset} onClick={openCalendly} size={"lg"}>
          ...
          <Col lg={3} md={3} sm={3}>
            <Card>
              <ExternalLink href={"mailto:help@fluidattacks.com"}>
                <Button icon={faEnvelope} size={"lg"}>
